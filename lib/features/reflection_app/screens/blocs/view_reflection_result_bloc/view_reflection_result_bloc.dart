...
class ViewReflectionResultBloc
  ...
  Future<void> _loadMoodReflectionsEvent(LoadMoodReflectionsEvent event,
    ...
    if (event.reflection != null) {
      ...
      try {
        final response = await reflectionService.getMoodReflections(
            event.topics, event.userReflections, event.heading);
  ...
  Future<void> _fetchReflectionHeadingEvent(FetchReflectionHeadingEvent event,
      Emitter<ViewReflectionResultState> emit) async {
    try {
      final heading =
          await fetchHeading(event.topics); // Use the refactored service method
